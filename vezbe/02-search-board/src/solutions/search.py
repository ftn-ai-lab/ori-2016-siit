...
# pratimo trenutnu dozvoljenu dubinu i ako ne nadjemo resenje a dodjemo do te dubine, onda je povecavamo
class IterativeDepthFirstSearch(Search):
    ...
    def search(self, initial_state):
        path = None
        while path is None and self.depth < self.board.rows * self.board.cols:
            path, processed, states = super(IterativeDepthFirstSearch, self).search(initial_state)
            self.depth += self.step
        self.depth = 20
        self.step = 20
        return path, processed, states

    def select_state(self, states):
        # TODO 2: Implementirati IDFS
        remove_states = deque([ state for state in states if state.depth > self.depth ])
        for state in remove_states:
            states.remove(state)
        ret_state = states.pop()
        if len(states) == 0:
            self.depth += self.step
        return ret_state
...
class GreedySearch(Search):
    def select_state(self, states):
        ...
        min_state = None
        for state in states:                                                        # trazimo stanje sa najmanjom cenom
            cost = state.get_cost()                                                 # i uklanjamo ga iz liste stanja
            if min_cost > cost:                                                     # gledamo samo procenjenu cenu
                min_cost = cost
                min_state = state
...
class AStarSearch(Search):
    def select_state(self, states):
        ...
        min_state = None
        for state in states:                                                        # trazimo stanje sa najmanjom cenom
            cost = state.get_cost() + state.depth                                   # i uklanjamo ga iz liste stanja
            if min_cost > cost:                                                     # gledamo procenjenu cenu kao
                min_cost = cost                                                     # i dosadasnju cenu
                min_state = state
        states.remove(min_state)
        return min_state
