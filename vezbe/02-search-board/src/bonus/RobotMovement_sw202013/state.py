"""
@author: Aleksa Zrnic      SW 20/2013
"""
        ...
        for d_row, d_col in zip(d_rows, d_cols):  # za sve moguce smerove
            ...
            if 0 <= new_row < self.board.rows and 0 <= new_col < self.board.cols and self.board.data[new_row][new_col] != 'w':
                if abs(d_row) > abs(d_col):
                    if self.board.data[row][new_col] != 'w' and self.board.data[row + d_row/2][new_col] != 'w':
                        new_positions.append((new_row, new_col))
                else:
                    if self.board.data[new_row][col] != 'w' and self.board.data[new_row][col + d_col/2] != 'w':
                        new_positions.append((new_row, new_col))
        return new_positions
...
class RookState(State):
    ...
    def get_legal_positions(self):
        ...
        for d_row, d_col in zip(d_rows, d_cols):
            new_row = row + d_row  # nova pozicija po redu
            new_col = col + d_col  # nova pozicija po koloni
            while 0 <= new_row < self.board.rows and 0 <= new_col < self.board.cols and self.board.data[new_row][new_col] != 'w':
                new_positions.append((new_row, new_col))
                new_row += d_row
                new_col += d_col
