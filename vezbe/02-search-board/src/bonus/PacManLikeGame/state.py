...
class State(object):
    ...
    @abstractmethod
    def __init__(self, board, parent=None, position=None, goal_position=None, no=0):
        """
        ...
        if self.parent is None:  # ako nema roditeljsko stanje, onda je ovo inicijalno stanje
            ...
            if len(agents) > 0:
                self.position = agents[no]  # pronadji pocetnu poziciju
...
class EnemyState(State):
    ...
    def __init__(self, board, parent=None, position=None, goal_position=None, no=0):
        super(self.__class__, self).__init__(board, parent, position, goal_position, no)
    ...
    def get_legal_positions(self):
        ...
        for d_row, d_col in zip(d_rows, d_cols):  # za sve moguce smerove
            ...
            if 0 <= new_row < self.board.rows and 0 <= new_col < self.board.cols and self.board.data[new_row][new_col] != 'w'\
                    and self.board.data[new_row][new_col] != self.get_agent_code() and self.board.data[new_row][new_col] != 'g':
